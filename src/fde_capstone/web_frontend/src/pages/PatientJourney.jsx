...
export default function PatientJourney({ patientKey, onPatientKeyChange, onOpenReadiness, onOpenAction, onRunAgent }) {
  ...
  return (
    <>
      ...
      <div className="hud-strip" style={{ marginBottom: 16 }}>
        <span className="hud-strip__live"><span className="pulse-dot" aria-hidden="true" /> Vein-to-vein HUD</span>
        <span className="hud-strip__sep" aria-hidden="true" />
        <span>Active subject <span className="hud-strip__id">{patientKey}</span></span>
        <span className="hud-strip__sep" aria-hidden="true" />
        <button
          type="button"
          title={EXAMPLE.note}
          className={`btn btn--sm ${EXAMPLE.key === patientKey ? 'btn--primary' : 'btn--outline'}`}
          onClick={() => open(EXAMPLE.key)}
        >
          {EXAMPLE.key}
        </button>
        <span className="subtle" style={{ letterSpacing: '0.02em', textTransform: 'none', fontWeight: 500 }}>{EXAMPLE.note}</span>
      </div>
